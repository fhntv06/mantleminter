    ...
    return (
      ...
      <div className="glass-card p-5.5">
        <div className="flex items-center gap-4 mb-6">
          ...
          />
          <div>
            <h2 className="text-lg sm:text-[2rem] text-black flex items-center gap-1">
        ...
        <div className="space-y-6">
          <div>
            <label className="block text-black text-sm mb-2">Address</label>
            <div className="relative">
              <input
                type="text"
                value={tokenAddress}
                readOnly
                className="w-full text-black placeholder-black border bg-[transparent] border-black rounded-full py-2.75 px-4 text-[0.925rem] pr-12"
              />
              <button
                onClick={(e) => {
                  e.stopPropagation();
            ...
            <p className="text-black text-xs mt-1 ml-4">On-chain smart contract address of the token parent.</p>
          </div>

          <div>
            <label className="block text-black text-sm mb-2">Admin</label>
            <div className="relative">
              <input
                type="text"
                value={owner ? `${owner.slice(0, owner.length / 2 - 3) + '...' + owner.slice(owner.length / 2 + 3)}` : ''}
                readOnly
                className="w-full text-black placeholder-black border bg-[transparent] border-black rounded-full py-2.75 px-4 text-[0.925rem]"
              />
              {isOwner && (
              <button
                className="absolute right-1 top-1/2 transform -translate-y-1/2 text-black h-[36px] w-[124px] rounded-full text-sm"
                onClick={(e) => {
                  e.stopPropagation();
            ...
            <p style={{color: 'rgba(197, 141, 0, 1)'}} className="text-black text-xs mt-1.5 ml-4 flex items-center">
          ...
          <div>
            <label className="block text-black text-sm mb-2">Total Supply</label>
            <div className="relative">
              <input
                type="text"
                value={`${formatNumberWithCommas(totalSupply) ?? 'null'} ${tokenSymbol}`}
                readOnly
                className="w-full text-black placeholder-black border bg-[transparent] border-black rounded-full py-2.75 pl-4 pr-[80px] text-[0.925rem]"
              />
              {isOwner && (
              <button
                className="absolute right-1 top-1/2 transform -translate-y-1/2 text-black h-[36px] w-[70px] rounded-full text-sm"
                onClick={(e) => {
                  e.stopPropagation();
          ...
          <div>
            <label className="block text-black text-sm mb-2">Wallet balance</label>
            <div className="relative">
              <input
                type="text"
          ...
          <div className="flex justify-center mt-4">
            <button
              ...
              className="border border-black text-black px-6 py-2 rounded-full text-[18px] transition-colors"
            >
              Add token to wallet
            </button>
          </div>
  ...
  return (
      <div className="glass-card rounded-3xl p-5.5">
        <div className="flex items-center gap-2 sm:gap-4 mb-3 sm:mb-4.5">
          ...
          />
          <div>
            <h2 className="text-lg sm:text-4xl text-black flex items-center gap-1">
              <span>{tokenName || 'Token Name'}</span>
        ...
        <div>
          <div>
            <input
              ...
              className="w-full text-black placeholder-black border bg-[transparent] border-black rounded-full py-2.75 px-4 text-[0.925rem]"
            />
            <p className="text-[0.725rem] text-black mt-1.75 ml-4">
              Your project unabbreviated name with spaces (usually 1-3 words).{' '}
              <button onClick={() => setTokenName('MantaMinter')} className="font-bold hover:text-black">
                Use example
              </button>
            </p>
          </div>
          ...
          <div className="mt-5">
            ...
            <p className="text-[0.725rem] text-black mt-1.75 ml-4">
              ...
              <button onClick={() => setTokenSymbol('MAM')} className="font-bold hover:text-black">
                Use example
              </button>
            </p>
          </div>
          <input
            className="w-full text-black placeholder-black bg-[transparent] border border-black rounded-full py-2.75 px-4 text-[0.925rem] mt-5"
            placeholder="Decimal"
            type="number"
            max="18"
            min="0"
          ...
          />
          <p className="text-[0.725rem] text-black ml-4">
            The decimal precision of your token (18 is Manta default).{' '}
            <button onClick={() => setDecimals(18)} className="font-bold hover:text-black">
              Use example
            </button>
          </p>
          ...
          <input
            ...
            className="w-full border text-black placeholder-black bg-[transparent] border-black rounded-full py-2.75 px-4 text-[0.925rem]"
          />

            <p className="text-[0.725rem] text-black mt-1.75 ml-4">
              Number of initial tokens to mint and send to your wallet address (float).{' '}
              <button onClick={() => setTokensToMint('1000000')} className="font-bold hover:text-black">
                Use example
              </button>
            </p>
          </div>
          ...
          <div className="flex justify-center mt-8">
            <button
              ...
              disabled={loading}
              className="ripple-button text-black px-4 sm:px-6 py-2 rounded-full flex items-center justify-center border min-w-40 border-black gap-2 sm:gap-3 text-[0.825rem] sm:text-[18px] transition-all duration-200"
              onMouseDown={(e) => {
